...
import { StreamingSummaryCard } from "@/components/dashboard/streaming-summary-card";
...
export default async function OverviewPage() {
  ...
  const firstName =
    ctx.user.displayName?.trim().split(/\s+/)[0] ||
    ...
    return (
      <div className="animate-fade-in">
        <PageHeader
          ...
          description="Here's what your fanbase is doing."
        />
        <EmptyState
          icon={<Users />}
          title="No fans yet."
  ...
  const [growth, sources, levels, cities, superfans, activity, streaming] =
    await Promise.all([
  ...
  return (
    <div className="animate-fade-in">
      ...
      <section
        className={`grid gap-3 sm:grid-cols-2 ${streaming ? "xl:grid-cols-5" : "xl:grid-cols-4"}`}
      >
        <Stat
          label="Identified fans"
          value={formatNumber(metrics.identified.value)}
          growth={metrics.identified.growth}
          hint="vs. 30 days ago"
        />
        <Stat
          label="Superfans"
          value={formatNumber(metrics.superfans.value)}
          growth={metrics.superfans.growth}
          hint={`Score ${formatNumber(metrics.superfanThreshold)}+`}
        />
        <Stat
          label="30-day active"
          value={formatNumber(metrics.active30.value)}
          growth={metrics.active30.growth}
          hint="vs. previous 30 days"
        />
        <Stat
          label="Revenue linked"
          value={formatMoney(metrics.revenue.valueCents, { compact: true })}
          growth={metrics.revenue.growth}
          hint={`${formatMoney(metrics.revenue.last30Cents, { compact: true })} in the last 30 days`}
        />
        {streaming ? (
          <Stat
            label="Spotify streams"
            value={formatNumber(streaming.streams.value, { compact: true })}
            growth={streaming.streams.growth}
            hint={`${formatNumber(streaming.monthlyListeners.value, { compact: true })} monthly listeners`}
          />
        ) : null}
      </section>
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-start justify-between">
            ...
            <span className="tabular text-xs text-subtle">
              +{formatNumber(growth.reduce((s, d) => s + d.newFans, 0))} new
            </span>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Share of interactions, last 30 days
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-3">
            {sources.length ? (
              <EngagementSourcesChart data={sources} />
            ) : (
              <p className="py-8 text-center text-sm text-subtle">
                No engagement yet in the last 30 days.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      {streaming ? (
        <section className="mt-4 grid gap-4 lg:grid-cols-3">
          <StreamingSummaryCard data={streaming} className="lg:col-span-2" />
          <Card>
            <CardHeader>
      ...
      <section
        className={`mt-4 grid gap-4 ${streaming ? "lg:grid-cols-2" : "lg:grid-cols-3"}`}
      >
        {!streaming ? (
          <Card>
            <CardHeader>
                  ...
                  {cities.map((c) => (
                    ...
                    >
                      <td className="py-2 pr-2">
                        <Link
                          href={`/app/fans?city=${encodeURIComponent(c.city ?? "")}`}
                      ...
                      <td className="tabular py-2 text-right text-muted-foreground">
                        {formatNumber(c.engaged)}
                      </td>
                      <td className="tabular py-2 text-right text-muted-foreground">
                        {formatNumber(c.attended)}
                      </td>
                      <td className="tabular py-2 text-right text-muted-foreground">
                        {formatNumber(c.spenders)}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <Card>
          <CardHeader className="flex-row items-start justify-between">
            ...
            <Link
              href={`/app/fans?minScore=${metrics.superfanThreshold}`}
          ...
          <CardContent className="px-0 pt-2 pb-2">
            {superfans.length === 0 ? (
              <p className="px-5 py-8 text-center text-sm text-subtle">
                No superfans yet — they will show up here as scores climb.
              </p>
            ) : (
              <ul>
                {superfans.map((f) => {
                  ...
                  return (
                    <li key={f.fanId}>
                      <Link
                        href={`/app/fans/${f.fanId}`}
                        className="flex items-center gap-3 px-5 py-2.5 transition-colors hover:bg-muted/60"
                      >
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Verified fan activity as it happens
              </CardDescription>
            </div>
          ...
          <CardContent className="px-0 pt-2 pb-2">
            {activity.rows.length === 0 ? (
              <p className="px-5 py-8 text-center text-sm text-subtle">
                Nothing yet. Activity will appear here as integrations sync.
              </p>
            ) : (
